...
class VendorContract(Base):
    """Vendor contract model for preferred pricing and terms"""
    ...
    is_active = Column(Boolean, default=True)
    document_url = Column(String(255))  # Link to uploaded contract PDF
    notes = Column(Text)
...
class VendorInvoice(Base):
    """Vendor invoice for purchase orders"""
...
class ReorderRule(Base):
    ...
    trigger_quantity = Column(Integer, nullable=False)  # Trigger when stock falls to or below this
    reorder_quantity = Column(Integer, nullable=False)  # Amount to order
    
    auto_approve = Column(Boolean, default=False)  # True = automatically create PO, False = just create alert
    is_active = Column(Boolean, default=True)
